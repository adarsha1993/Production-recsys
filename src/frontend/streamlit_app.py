...
.nf-hero {{
  ...
  background: linear-gradient(180deg, transparent 0%, {BG} 100%),
              linear-gradient(90deg, {'rgba(0,0,0,0.9)' if dark else 'rgba(0,0,0,0.5)'} 0%, transparent 60%);
...
.nf-card-overlay {{
  ...
  background: linear-gradient(transparent 0%, {CARD_OVER} 100%);
...
.match-badge {{
  ...
  color: #a3cf62; letter-spacing: 0.02em;
}}
.type-hstu {{
...
.src-cache {{
  ...
  color: #a3cf62; font-family: 'Inter', sans-serif;
  font-size: 11px; font-weight: 600;
  padding: 5px 14px; border-radius: 3px;
}}
.src-model {{
  display: inline-flex; align-items: center; gap: 6px;
  background: {'rgba(229,9,20,0.12)' if dark else 'rgba(229,9,20,0.08)'};
...
.nf-stat:hover {{ border-color: {ACCENT}; }}
.nf-stat-val {{
  font-family: 'Inter', sans-serif;
  font-size: 32px; font-weight: 900;
  color: {TEXT_HI}; letter-spacing: -0.03em; line-height: 1;
}}
.nf-stat-lbl {{
  font-family: 'Inter', sans-serif;
  font-size: 11px; font-weight: 500;
  color: {TEXT_LO}; letter-spacing: 0.08em;
...
.nf-hist:hover {{ background: {HOVER_BG}; }}
.nf-hist:last-child {{ border-bottom: none; }}
.nf-hist-title {{
  font-family: 'Inter', sans-serif;
  font-size: 14px; font-weight: 500; color: {TEXT_HI};
}}
.nf-hist-meta {{
  font-family: 'Inter', sans-serif;
  font-size: 12px; color: {TEXT_LO}; margin-top: 2px;
}}
.nf-match {{
  font-family: 'Inter', sans-serif;
  font-size: 13px; font-weight: 700;
  color: #a3cf62; white-space: nowrap;
}}
.nf-svc {{
  display: flex; align-items: center;
  justify-content: space-between;
...
.nf-svc-name {{
  font-family: 'Inter', sans-serif;
  font-size: 13px; font-weight: 600; color: {TEXT_HI};
}}
.nf-svc-port {{
  font-family: 'JetBrains Mono', monospace;
  font-size: 11px; color: {TEXT_LO};
}}
.dot-on {{
  display: inline-block; width: 8px; height: 8px;
  border-radius: 50%; background: #a3cf62;
...
.nf-progress {{
  background: {BG3}; border-radius: 2px;
  height: 4px; overflow: hidden; margin: 14px 0 10px;
}}
.nf-progress-fill {{
  height: 100%;
...
.nf-meta {{
  ...
  flex-wrap: wrap; padding: 10px 0 18px;
}}
.nf-meta-item {{
  font-family: 'Inter', sans-serif;
  font-size: 12px; font-weight: 500; color: {TEXT_LO};
}}
.sb-logo {{
  display: flex; align-items: center;
...
.sb-logo-text {{
  ...
  color: {TEXT_HI}; letter-spacing: -0.01em; line-height: 1.2;
}}
.sb-logo-sub {{
  font-family: 'Inter', sans-serif;
  font-size: 10px; font-weight: 400;
  color: {TEXT_LO}; letter-spacing: 0.02em;
}}
.sb-section {{
  font-family: 'Inter', sans-serif;
  font-size: 10px; font-weight: 700;
  color: {TEXT_LO}; letter-spacing: 0.12em;
  text-transform: uppercase;
...
.sb-link {{
  ...
  color: {TEXT_MID}; text-decoration: none;
...
.no-content {{
  ...
  font-family: 'Inter', sans-serif;
  font-size: 14px; color: {TEXT_LO};
}}
.err-box {{
  background: {'#2a0a0a' if dark else '#fff5f5'};
  border: 1px solid {'#6a1a1a' if dark else '#fca5a5'};
  border-radius: 4px; padding: 20px 24px;
  font-family: 'Inter', sans-serif; font-size: 13px;
  color: {'#ff6b6b' if dark else '#dc2626'}; line-height: 1.7;
}}
</style>
...
def get_poster(row) -> Optional[str]:
    """Get poster URL from row or dict"""
    try:
        # Handle dict (from API rec)
        if isinstance(row, dict):
            p = str(row.get('poster', '')
                    or row.get(
                        'poster_path', ''))
        else:
            p = str(row.get(
...
def match_pct(score: float,
              rank:  int = 1) -> str:
    """Netflix-style match percentage"""
    if score > 0.001:
        base = 98 - (rank - 1) * 2
        pct  = max(min(base, 98), 75)
    else:
        random.seed(rank * 13)
        pct = random.randint(72, 88)
    return f"{pct}% Match"
...
""", unsafe_allow_html=True)
    ...
    for name, url, icon in [
        ("API Docs",
         "https://production-recsys"
         ".onrender.com/docs", "📋"),
        ("GitHub",
         "https://github.com/adarsha1993"
         "/Production-recsys", "💻"),
        ("Prometheus",
         "http://localhost:9090", "📈"),
        ("Grafana",
...
st.markdown(
    ...
    'center;gap:16px;margin-top:24px;'
    'padding-top:20px;border-top:1px solid '
    + BORDER + ';">'

    '<div style="width:44px;height:44px;'
    'border-radius:50%;background:linear-'
    'gradient(135deg,' + ACCENT +
    ' 0%,#ff6b35 100%);display:flex;'
    'align-items:center;justify-content:'
    'center;font-family:Inter,sans-serif;'
    'font-size:18px;font-weight:900;'
    'color:#ffffff;flex-shrink:0;">A</div>'

    '<div>'
    '<div style="font-family:Inter,'
    'sans-serif;font-size:15px;'
    'font-weight:800;color:' + TEXT_HI +
    ';letter-spacing:-0.01em;'
    'line-height:1.2;">Adarsha Ghimire</div>'
    '<div style="font-family:Inter,'
    'sans-serif;font-size:12px;'
    'font-weight:400;color:' + TEXT_LO +
    ...
    ';letter-spacing:0.05em;">CP612</div>'
    '<div style="font-family:Inter,'
    'sans-serif;font-size:10px;'
    'color:' + TEXT_LO + ';margin-top:2px;">'
    'Production Recommendation Systems'
    '</div>'
    '<div style="font-family:Inter,'
    'sans-serif;font-size:10px;'
    'color:' + TEXT_LO + ';margin-top:1px;">'
    '© Adarsha Ghimire'
    '</div></div>'
...
with tab1:
    ...
    if "error" in rec_data:
        ...
        st.markdown(
            '<div class="nf-meta">'
            + src_badge +
            f'<span class="nf-meta-item">'
            f'{lat:.0f}ms</span>'
            f'<span class="nf-meta-item">'
            f'{len(recs)} titles</span>'
            f'<span class="nf-meta-item"'
            f' style="color:{TEXT_LO};">'
            f'{rid}</span>'
            '</div>',
            unsafe_allow_html=True)
...
with tab2:
    ...
    if ratings.empty:
        st.markdown(
            '<div class="no-content">'
            'No ratings data found.<br>'
            'Check data files are in repo.'
            '</div>',
            unsafe_allow_html=True)
    else:
        ur = ratings[
...
with tab3:
    ...
    def make_row(layer, title,
        ...
        return (
            ...
            'flex-shrink:0;font-family:'
            'Inter,sans-serif;font-size:10px;'
            'font-weight:700;color:'
            + TEXT_LO +
    ...
    rows += make_row(
        ...
        ["Python 3.12", "Netflix-style UI",
         "Dark / Light Mode", "TMDB Posters",
         "Real-time Metrics",
         "Inline Feedback"],
        border_top=False)
    rows += make_row(
        ...
        ["Rate Limiting 100/min",
         ...
         "Prometheus Metrics"])
    rows += make_row(
        "Model Serving", "BentoML 1.x",
        ":3001",
        ["HSTU Ranker", "PyTorch Inference",
         "ONNX Export", "Popularity Padding",
         "Cold Start Fallback",
         "Deduplication"])
    rows += make_row(
        "Cache Layer", "Redis 7", ":6379",
        ["Per-user Caching", "TTL 1 Hour",
    ...
    rows += make_row(
        ...
        ["6 Custom Metrics",
         "p50 / p95 / p99",
         "Cache Hit Rate", "Request Volume",
         "Error Rate", "Evidently AI Drift",
         "15s Scrape Interval",
         "7-Panel Dashboard"])
    rows += make_row(
        "Data & MLOps",
    ...
    rows += make_row(
        ...
        ["GRank (WWW 2026)",
         ...
         "Qdrant HNSW ANN",
         "e5-large Embeddings",
         "CLIP Multimodal", "Ollama RAG",
         "500 Candidates"])
    rows += make_row(
        "Ranking", "Neural Rankers", "",
        ["HSTU", "Netflix FM", "OneRec",
         "LightGCN", "BERT4Rec",
         "DPO Alignment", "Multi-task",
         "MMR Re-ranking", "IPS Debiasing",
         "FM-Intent", "LLM Explainability",
    ...
    rows += (
        ...
        ';letter-spacing:0.15em;'
        'text-transform:uppercase;'
        'padding-top:6px;">Key Results</div>'
